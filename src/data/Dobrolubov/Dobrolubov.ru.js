exports.frontmatter = {
  path: '/Dobrolubov',
  name: 'Игорь Михайлович Добролюбов',
  date: '22 октября 1933 - 19 июля 2010',
  img: 'dobrolubov/dobrolubov.jpg',
  videoId: 'wNuYlYKSylU',
  vita: 'Белорусский советский кинорежиссёр, актёр, педагог и сценарист, народный артист Белорусской ССР (1985). Член КПСС с 1963 года',
  authorOfTheDay: 'true',
  lang: 'ru',
  birthPlace: 'г. Новосибирск, Российская Федерация',
  timelineData: [
    {
      date: '1956',
      text:
                'Окончил факультет журналистики Белорусского государственного университета.',
    },
    {
      date: '1956—1958',
      text:
                'Актёр Белорусского республиканского ТЮЗа.',
    },
    {
      date: '1963',
      text:
                'Всероссийский государственный институт кинематографии имени Герасимова (мастерская Михаила Ромма).',
    },
    {
      date: 'с 1963',
      text:
                'Режиссёр киностудии «Беларусьфильм».',
    },
    {
      date: '1974 - 1975',
      text:
                'Секретарь правления Союза кинематографистов БССР.',
    },
    {
      date: '1993',
      text:
                'Профессор Института современных знаний имени А. М. Широкова (Минск).',
    },
  ],

  work: [
    {
      title: 'Мама, я жив!',
      date: '1985',
      img: '../assets/images/dobrolubov_mama_ya_zhiv.jpg',
    },
    {
      title: 'Белые росы',
      date: '1984',
      img: '../assets/images/dobrolubov_belye_rosi.jpg',
    },
    {
      title: 'Расписание на послезавтра',
      date: '1978',
      img: '../assets/images/dobrolubov_raspisanie_na_poslezavtra.jpg',
    },
    {
      title: 'По секрету всему свету',
      date: '1976',
      img: '../assets/images/dobrolubov_po_secrety_vsemy_svety.jpg',
    },
    {
      title: 'Братушка',
      date: '1975',
      img: '../assets/images/dobrolubov_bratushka.jpg',
    },
    {
      title: 'Иван Макарович',
      date: '1968',
      img: '../assets/images/dobrolubov_ivan_makarovich.jpg',
    },
    {
      title: 'Иду искать',
      date: '1966',
      img: "../assets/images/dobrolubov_idu_iskat'.jpg",
    },
  ],
};
